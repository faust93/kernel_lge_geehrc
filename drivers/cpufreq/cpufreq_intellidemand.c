...
static inline cputime64_t c_get_cpu_idle_time(unsigned int cpu, cputime64_t *wall)
{
				      ...
				      const char *buf, size_t count)
{
	...
	for_each_online_cpu(j) {
		struct cpu_dbs_info_s *dbs_info;
		dbs_info = &per_cpu(id_cpu_dbs_info, j);
		dbs_info->prev_cpu_idle = c_get_cpu_idle_time(j,
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		cur_idle_time = c_get_cpu_idle_time(j, &cur_wall_time);
...
static void dbs_refresh_callback(struct work_struct *work)
{
	...
	if (policy->cur < policy->max) {
		...
		this_dbs_info->prev_cpu_idle = c_get_cpu_idle_time(cpu,
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			j_dbs_info->prev_cpu_idle = c_get_cpu_idle_time(j,
...
static int cpufreq_gov_dbs_up_task(void *data)
{
	...
	while (1) {
		...
		if (policy->cur < input_event_min_freq) {
			...
			this_dbs_info->prev_cpu_idle = c_get_cpu_idle_time(cpu, &this_dbs_info->prev_cpu_wall);
		}
